            ...
            <Card key={request.id} className="p-6 hover:shadow-md transition-shadow">
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      <h3 className="text-lg font-bold text-slate-800">{request.company}</h3>
                    ...
                    <p className="text-sm text-slate-600 mb-1">
                      <Icon name="Building2" className="inline mr-1" size={14} />
                      {request.object_name}
                    </p>
              ...
              <div className="grid md:grid-cols-3 gap-4 mb-4">
                <div className="bg-slate-50 rounded-lg p-4">
                  ...
                  <p className="font-semibold text-slate-800 mb-1">{request.full_name}</p>
                ...
                <div className="bg-slate-50 rounded-lg p-4">
                  <div className="flex items-center gap-2 mb-2">
                    <Icon name="MapPin" className="text-slate-400" size={16} />
                    <p className="text-xs font-semibold text-slate-500 uppercase">Объект</p>
                  </div>
                  <p className="font-semibold text-slate-800 mb-2">{request.object_address}</p>
                ...
                {request.status === 'completed' && request.pool_size && (
                  <div className="bg-blue-50 rounded-lg p-4">
                    <div className="flex items-center gap-2 mb-2">
                      <Icon name="Waves" className="text-blue-500" size={16} />
                      <p className="text-xs font-semibold text-blue-700 uppercase">Бассейн</p>
                    </div>
                    <p className="text-sm text-slate-700">{request.pool_size}</p>
                  </div>
                )}
              </div>
                ...
                <div className="border-t pt-4 space-y-4">
                  {request.visitors_info && (
                    <div>
                      ...
                      <p className="text-sm text-slate-700 bg-slate-50 rounded p-3">{request.visitors_info}</p>
                    </div>
                  )}
                  ...
                  {request.deadline && (
                    <div>
                      ...
                      <p className="text-sm text-slate-700 bg-slate-50 rounded p-3">{request.deadline}</p>
                    </div>
                  )}
                      ...
                      <div className="grid md:grid-cols-2 gap-3">
                        {request.company_card_url && (
                          <a 
                            ...
                            className="flex items-center gap-3 p-3 bg-slate-50 rounded-lg hover:bg-primary/10 hover:border-primary border border-slate-200 transition group"
                          >
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <p className="text-xs text-slate-500">Открыть файл</p>
                            </div>
                            <Icon name="ExternalLink" className="text-slate-400 group-hover:text-primary transition" size={16} />
                          </a>
                        )}
                        {request.pool_scheme_urls && request.pool_scheme_urls.map((url, idx) => (
                          ...
                          >
                            ...
                            <Icon name="ExternalLink" className="text-slate-400 group-hover:text-blue-600 transition" size={16} />
                          </a>
                        ))}
                      </div>
                    </div>
